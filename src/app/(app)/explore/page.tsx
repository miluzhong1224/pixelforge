    ...
    <div className="max-w-screen-2xl mx-auto px-6 py-6">
        ...
        <div className="flex items-center gap-3">
          <div className="relative">
            ...
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="搜索作品..."
              className="h-9 w-48 pl-9 pr-3 rounded-lg bg-zinc-800/50 border border-zinc-700 text-sm text-zinc-200 placeholder:text-zinc-500 focus:outline-none focus:ring-2 focus:ring-violet-500" />
          </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-20">
          <p className="text-zinc-500">{search ? '没有匹配的作品' : '暂无公开作品'}</p>
        </div>
      ) : (
              ...
              <div className="relative">
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-zinc-950/90 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex flex-col justify-end p-3">
                  ...
                  <div className="flex items-center justify-between">
                    <Link href={`/user/${img.user_id}`} onClick={e => e.stopPropagation()} className="text-[11px] text-zinc-400 hover:text-violet-400 transition-colors">{img.user_name}</Link>
                    <div className="flex items-center gap-1.5">
                      <button onClick={(e) => { e.stopPropagation(); setCommentImgId(img.id); }}
                        className="p-1 rounded-md text-[11px] text-zinc-400 hover:text-zinc-200 transition-colors">
