...
export default function UserPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="max-w-screen-2xl mx-auto px-6 py-6">
      <button onClick={() => router.back()} className="p-2 rounded-lg text-zinc-500 hover:text-zinc-300 hover:bg-zinc-800/50 mb-6"><ArrowLeft size={20} /></button>

      {/* 用户信息头部 */}
      <div className="flex items-center gap-4 mb-8 p-6 rounded-2xl bg-zinc-900/50 border border-zinc-800">
        <div className="h-16 w-16 rounded-full bg-violet-600 flex items-center justify-center text-white text-2xl font-bold">{displayName[0]?.toUpperCase()}</div>
        <div>
          <h1 className="text-xl font-bold text-zinc-100">{displayName}</h1>
          <p className="text-sm text-zinc-500 mt-1">{images.length} 张公开作品</p>
        </div>
      </div>

      {loading ? (
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
          {Array.from({ length: 6 }).map((_, i) => (<div key={i} className="aspect-square rounded-xl bg-zinc-800/50 animate-pulse" />))}
        </div>
      ) : images.length === 0 ? (
        <div className="text-center py-20">
          <ImageIcon size={40} className="mx-auto text-zinc-700 mb-3" />
          <p className="text-zinc-500">暂无公开作品</p>
        </div>
      ) : (
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
          {images.map((img) => (
            <div key={img.id} className="rounded-xl overflow-hidden border border-zinc-800 bg-zinc-900/50 hover:border-zinc-700 transition-all group">
              <div className="relative aspect-square">
                ...
                <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                  <button onClick={(e) => { e.stopPropagation(); router.push(`/generate?prompt=${encodeURIComponent(img.prompt)}`); toast.success('Prompt 已填入'); }}
                    className="flex items-center gap-1 px-3 py-1.5 rounded-lg bg-violet-600/80 text-white text-xs hover:bg-violet-500"><Copy size={12} />复用</button>
                </div>
              </div>
              <div className="p-2">
                <p className="text-[11px] text-zinc-500 line-clamp-1">{img.prompt}</p>
              </div>
            </div>
          ))}
        </div>
