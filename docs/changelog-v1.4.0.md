...
| 06-25 | 广场空白：`like_count`/`reuse_count` 列不存在导致查询失败 | 移除缺失列，select 仅查已有字段 |
| 06-25 | 公开按钮点击后广场不显示 | Vercel API 超时 → 浏览器直连 Supabase PATCH |
| 06-25 | 扩图旧图片加载失败（CORS / 链接过期） | 三层降级：页面 ref → Vercel 代理 → fetch blob |
| 06-25 | 扩图 Canvas 污染报错 `Tainted canvases` | `isTainted()` 自动检测 + `loadImage()` 降级 |
| 06-25 | 广场点赞功能 | ❤️ 点击交互 + 本地状态（列不存在时纯前端） |
| 06-25 | 变更日志补充 | 全版本 changelog 同步到仓库 `docs/` 目录 |
